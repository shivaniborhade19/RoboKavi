...
class Future(object):
    ...
    method:`run` will also propogate exceptions; see it's docstring for details.
    """
    ...
    def wait(self):
        ...
        `wait` blocks until the result is ready (either :method:`result` or
        :method:`exception` has been called), and then returns it (in the case
        of :method:`result`, or raises it (in the case of :method:`exception`).
        """
        with self._lock:
    ...
    def result(self, value):
        ...
        :param:`value` is the result to supply; it will be returned when
        :method:`wait` is called.
        """
        self._value = value
        self._lock.release()
    ...
    def exception(self, err):
        ...
        :param:`e` is an exception, which will be raised when :method:`wait`
        is called.
        """
        self._exception = err
        self._lock.release()
    ...
    def run(self, task):
        ...
        If :param:`task` rasies an exception, pass it to :method:`exception`.
        Otherwise, pass the return value to :method:`result`.
        """
        try:
            self.result(task())
...
class DebouncedSyncVar(object):
    """A synchronized variable, which debounces its value

    DebouncedSyncVar supports three operations: put, replace, and get.
    ...
    def get(self, blocking=True, remove=True):
        """Retrieve a tuple (value, ok).

        ``value`` is the value of the variable (if successful, see below),
        and ok indicates whether or not a value was successfully retrieved.

        If :param blocking: is True, block until (1) the dsv has a value
        ...
        are not met, return ``(None, False)`` immediately.

        If :param remove: is True, remove the value when returning it.
    ...
    def replace(self, value):
        """Replace the current value of the dsv (if any) with :param:`value`.
    ...
    def put(self, value):
        """Set the dsv's value to :param:`value`.
...
def in_mainloop(func):
    ...
    `in_mainloop` exists because Gtk isn't threadsafe, and therefore cannot be
    manipulated except in the thread running the Gtk main loop. `in_mainloop`
...
def mainloop_only(f):
    ...
    `mainloop_only` should be used to decorate functions which are unsafe
...
def holding(lock, task, blocking=True):
    ...
    If :param:`blocking` is True, wait for the lock before running.
    Otherwise, if the lock is busy, return None immediately, and don't
    spawn `task`.

    The return value is a future which can be used to retrive the result of
    running task (or None if the task was not run).
    """
...
def run_or_none(func):
    ...
    See holding() above with blocking=False
    """
    lock = threading.Lock()
